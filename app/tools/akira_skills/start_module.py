...
def register(mcp: FastMCP) -> None:
    @mcp.tool
    def start_service_module(service_name: str) -> str:
        """Start a service module from the service hub row by name."""
        with sync_playwright() as p:
            ...
            try:
                ...
                status_text = ""
            if status_text in {"encendido", "en marcha", "activo", "running", "on"}:
                browser.close()
                return f"Service module '{service_name}' already started ✅"
            if row.locator(
            ...
            start_btn = row.locator("button:has(i:has-text('play_arrow'))").first
            if start_btn.count() == 0:
                start_btn = row.locator("button.rz-success").first
            if start_btn.count() == 0:
                raise RuntimeError(f"Start button not found for '{service_name}'")
        ...
        return f"Service module '{service_name}' start requested ✅"
